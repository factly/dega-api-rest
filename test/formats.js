...
describe('/api/v1/formats', () => {
    ...
    it('Should get all formats', () => {
        return request(mock)
            .get('/api/v1/formats?clientId=Factly')
            .expect(200)
            .expect('Content-Type', /json/)
            .then((res) => {
                const clientId = JSON.parse(res.text).data;
                expect(clientId.length).eq(4);
            });
        });
    it('Should get all formats', () => {
        return request(mock)
            .get('/api/v1/formats?slug=audio')
            .expect(200)
            .expect('Content-Type', /json/)
            .then((res) => {
                const clientId = JSON.parse(res.text).data;
                expect(clientId.length).eq(1);
            });
    });
});
